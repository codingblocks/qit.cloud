...
exports.callback = function (feedResults, context = console) {
  if(!(feedResults && feedResults.updateFeed && feedResults.updateFeed.value && feedResults.errors)) {
    context.error('Invalid format specified');
    return;
  }

  context.log(`## ${feedResults.feedUrl}`);
  context.log(`* ${feedResults.updateFeed.value.length} episodes successfully parsed`);
